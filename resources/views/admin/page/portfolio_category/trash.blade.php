    ...
    <div class="col-lg-9">
        <div class="card">
            <div class="card-header">
                ...
                <a class="float-right text-danger" href="{{ route('portfolio-category.index') }}">Back To Home<i class="fa fa-arrow-right"></i></a>
            </div>
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table mb-0 data-table-ov">
                        ...
                        <tbody>
                           @forelse ($trash_category as $trash)
                               <tr>
                                    <td>{{ $loop -> index + 1 }}</td>
                                    <td>{{ $trash -> name }}</td>
                                    <td>{{ $trash -> slug }}</td>
                                    ...
                                    <td>
                                    @if ($trash -> trash)
                                        <a class="" href="{{ route('Pc.trash.update',$trash -> id,) }}"><span class="badge badge-info">Restore User</span></a>
                                    @endif
                                </td>
                                <td>
                                    <form  class="d-inline" action="{{ route('portfolio-category.destroy',$trash -> id ) }}" method="post">
                                        @csrf
                                        @method('DELETE')
