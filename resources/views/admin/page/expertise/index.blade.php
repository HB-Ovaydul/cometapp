    ...
    <div class="col-lg-9">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title d-inline justify-content-between">All Expertise</h4>
                <a class="float-right text-danger" href="{{ route('ex.move.trash') }}">Trash <i class="fa fa-arrow-right"></i></a>
            </div>
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table mb-0 data-table-ov">
                        ...
                        <tbody>
                           @forelse ($all_exper as $exper)
                               <tr>
                                    <td>{{ $loop -> index + 1 }}</td>
                                    <td>{{ $exper -> brand_name }}</td>
                                    <td>@if (isset( $exper -> ex_photo))
                                      <img style="width: 60px;height:60px;object-fit:cover;" src="{{ url('storage/expertise/'. $exper -> ex_photo) }}" alt="">
                                      @else
                                         No Photo! 
                                    @endif
                                  
                                  </td>
                                    <td><i class="{{ $exper -> brand_logo }}"></i></td>
                                    <td>{{ $exper -> created_at -> diffForHumans() }}</td>
                                    <td>
                                        @if ($exper -> status)
                                            <span class="badge badge-success">Active User</span>
                                            <a class="text-danger" href="{{ route('ex.status.update',$exper -> id,) }}"><i class="fa fa-times"></i></a>
                                        @else
                                            <span class="badge badge-danger">Blocked User</span>
                                            <a class="text-success" href="{{ route('ex.status.update',$exper -> id) }}"><i class="fa fa-check"></i></a>
                                        @endif
                                    </td>
                                    <td>  
                                        <a class="btn btn-sm btn-warning" href="{{ route('expertise.edit',$exper -> id) }}"><i class="fa fa-edit"></i></a>
    
                                     <a class="btn btn-sm btn-danger" href="{{ route('ex.Update.trash', $exper -> id ) }}"><i class="fa fa-trash"></i></a>
                                    </td>
                             </tr>
                           @empty
                               <tr>
                ...
                <form action="{{ route('expertise.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label>Paragraph</label>
                            <textarea name="paragraph" rows="5" cols="5" class="form-control" placeholder="Enter text here">      
                        </textarea>
                    </div>
      ...
      <div class="card flex-fill">
          ...
          <div class="card-body">
              <form action="{{ route('expertise.update', $edit_id -> id) }}" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                      <label>BrandName</label>
                      <input name="brand_name" type="text" value="{{ $edit_id -> brand_name}}" class="form-control">
                  </div>
                  <div class="form-group">
                      <label>Paragraph</label>
                          <textarea name="paragraph" rows="5" cols="5" class="form-control" placeholder="Enter text here">{{ $edit_id -> brand_paragraph}}     
                      </textarea>
                  </div>

                      <div class="form-group">
                          <label><button class="btn btn-sm btn-primary btn-select-modal">Select Icon</button></label>
                          <input name="icon" type="text" value="{{$edit_id -> brand_logo}}" class="form-control ov-icon-select">
                      </div>
                      <div class="form-group">
                          <label>Expertise Photo</label>
                          <br>
                          <input style="display: none;" id="photo-id" name="new_photo" type="file" class="form-control">
                          <input style="display: none;" id="photo-id" name="old_photo" type="file" class="form-control">
